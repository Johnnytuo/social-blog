...
function updateList(responseText){
    console.log("updatelist" +responseText);
    ...
    //
    // var html_posts_num = parseInt(document.getElementsByClassName("post_content_text")[0].id.replace("id_post_text_",""));
    ...
    // if(id>html_posts_num){
        ...
        " <em>Post by </em>"+
        "<a href='/socialnetwork/profile/"+ this.user_id+"' id='id_post_profile_"+this.id+"' class='profile_link'>"+this.username+"  </a>-"+
        "<span class='post_content_text' id='id_post_text_"+this.id+"'>"+this.post_text +" </span>- "+
        ...
        "<input id='id_comment_text_input_"+this.id+"' type='text' name='comment'>"+
        "<button class = 'comment_button' id ='id_comment_button_"+this.id+"' onclick='addcomment("+this.id+"')>submit</button><br><br>"+
        "<span id='error' class='error'></span></div>"

        );

        //event delegation
        $( document ).on( "click", ".comment_button", {para: this.id},function (event) {
                addcomment(event.data.para);
        });
    // }
       ...
       var time_format=transfer_time(this.time);
        ...
        // if(id > html_comments_num){
         list.prepend(
                ...
                "<span class='comment_content_text' id='id_comment_text_"+this.id+"'>"+this.comment_text +" </span>- "+
...
function addcomment(post_id) {
    ...
    // var str = "id_comment_text_"+post_id
    var commentElement = $("input#id_comment_text_input_"+post_id);
